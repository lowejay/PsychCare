...
function removethisEmployee(id) {
    if (confirm('Confirm removing this employee? This action cannot be undone.')) {
        window.location = '../controllers/admin_controller.php?this-employee=' + id;
    } else {
        return false;
    }
}
...
function addthisStudent(sid,eid,cid) {

    $(document).ready(function() {
            $.ajax({
            url: '../controllers/teacher_controller.php',
            type: "POST",
            data: { sid: sid, eid: eid, cid: cid },
            success: function (d) {
                if (d === 'false') {
                    swal({
...
function removethisSchedule(id) {
    swal({
        ...
        .then((remove) => {
            if (remove) {
                window.location = '../controllers/teacher_controller.php?this-schedule=' + id;
            } else {
...
function removethisStudent(cid,id) {
    swal({
        ...
        .then((remove) => {
            if (remove) {
                window.location = '../controllers/teacher_controller.php?this-student=' + id + '&this-class=' + cid;
            } else {
...
function validateScheduleTime() {
    $(document).ready(function() {
        ...
        var time2 = $(".time-until").val();
        if (time >= time2) {
            var $str = "Time-end must be greater than time start!";
            swal($str, {
                button: "Ok",
            })
            .then(function() {
...
function validateAppointmentTime() {
    $(document).ready(function() {
        ...
        var time2 = $(".time-until").val();
        if (time < time2) {
            $(".time-until").blur(function() {
                $(".submit_sched").prop("disabled", false);
            });
        } else if (time >= time2) {
            ...
            swal($str, {
                ...
                .then(function() {
                    $(".submit_sched").prop("disabled", true);
